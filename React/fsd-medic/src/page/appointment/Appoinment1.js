...
function Appoinment() {
  ...
  useEffect(() => {
    ...
    if (user != "") {
      Axios.get(PATH + "/appointment?email=" + user).then((response) => {        
  ...
  const btn_del = (item) => {
    if (item.status == 0) {
      ...
      alert(
        "The appointment has already confirmed. Please contact your doctor."
      );
    }
  };
                  ...
                  {list && list.map((item, key) => (
                    <tr key={item.id}>
                      ...
                      <td width="10%">
                        ...
                        &nbsp;&nbsp;
                        {item.status == 0 ? (
                          <Link type="button" onClick={() => btn_del(item)}>                            
                            Delete
                          </Link>
                        ) : null}
                      </td>
